...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sampleArray = data.samples[0];

    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var desiredSampleNumber = sampleArray.filter(sampleObj => sampleObj.id == sample);

    //  5. Create a variable that holds the first sample in the array.
    var firstSampleArry = desiredSampleNumber[0];

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var id = firstSampleArry.otu_ids;
    var label = firstSampleArry.otu_labels;
    var values = firstSampleArry.sample_values;

    // 7. Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 

    var yticks = id.slice(0, 10).map(otuID => 'OTU' + id).reverse();
